...
class LoginActivity : AppCompatActivity(), Callback<Response> {
    ...
    //lateinit var sharedPrefUserName:String
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnLogin.setOnClickListener {
            ...
            if(userName.isNullOrEmpty()){
                showToast("The username can't be empty.")
            }else{
                progressDialog.show()
                val request = Request(
                    action = Constant.GET_USER,
                    userName = userName
                )
    ...
    override fun onResponse(call: Call<Response>, response: retrofit2.Response<Response>) {
        ...
        if(response.body()!=null){
            ...
            if(serverResponse!!.status){
                saveUserToPref(serverResponse.userId,serverResponse.userName)
